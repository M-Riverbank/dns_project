...
//      .option("kafka.bootstrap.servers", "dsy:9092") //指定kafka集群的地址，理论上写一个broker就可以了
...
//    val ds: DataFrame = rawDF.selectExpr("CAST(value AS STRING)") //将kafka中的数据的value转为为string，原始为binary类型
//      .map(row => {
