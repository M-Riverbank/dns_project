...
//      .builder()
//      .config(conf)
//      .config("spark.sql.hive.hiveserver2.jdbc.url","jdbc:hive2://dsy:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2")
//      .config("spark.datasource.hive.warehouse.metastoreUri","thrift://dsy:9083")
...
//    val ds: DataFrame = rawDF
//      .selectExpr("CAST(value AS STRING)")  //将kafka中的数据的value转为为string，原始为binary类型
//      .map(row => {
...
//      .option("checkpointLocation","hdfs://dsy/tmp/offset/test/StructuredStreamingFromKafka2Hive") /**用来保存offset，用该目录来绑定对应的offset，如果该目录发生改变则程序运行的id会发生变化，类比group.id的变化，写hive的时候一定不要轻易变动*/
//      .partitionBy("year","month","day")//提供分区字段
//      .toTable("test")//写入hive表
